...
ggplot(data = AccaData, aes(x = as.character(Date), y = Win, fill = HomeAway)) + geom_bar(stat = "identity") + theme_classic() + labs(title = "Home/Away tracker") + scale_y_continuous(breaks = 1:11) #bar plot showing split of home or away picks for winning bets
...
#popularity of teams
library(treemapify)
AccaData %>% group_by(Team) %>% summarise(Picked = n()) %>% ggplot(aes(area = Picked, fill = Team, label = paste(Team, Picked, sep = "\n"))) + geom_treemap() +   geom_treemap_text(place = "centre") + theme(legend.position = "none") + labs(title = "Popular teams to bet on")
#collapse teams picked once into a single category

AccaData %>% group_by(Against) %>% summarise(Picked = n()) %>% ggplot(aes(area = Picked, fill = Against, label = paste(Against, Picked, sep = "\n"))) + geom_treemap() +   geom_treemap_text(place = "centre") + theme(legend.position = "none") + labs(title = "Popular teams to bet against")
```
