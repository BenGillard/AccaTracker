...
CurrentStreak<- AccaData %>% group_by(Player) %>% filter(Win == 0) %>% mutate(LastLoss = -1*as.numeric((last(Date) - last(AccaData$Date))/7)) %>% distinct(Player, .keep_all = TRUE) #work out how many days since last win

PlayerSummary$Streak<- CurrentStreak$LastLoss[match(x = PlayerSummary$Player, table = CurrentStreak$Player)]
