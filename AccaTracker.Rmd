...
CloseCalls<- GroupWins %>% filter(Wins == 10)
library(kableExtra)
cbind(AccaData[which(AccaData$Date %in% CloseCalls$Date & AccaData$Win == 0),1:6], CostofLoss = paste0("£",CloseCalls$Return)) %>% kbl(caption = "Table of shame. When we were one result away from the jackpot", row.names = FALSE) %>% kable_classic(html_font = "Cambria") %>% kable_styling("striped")
```
