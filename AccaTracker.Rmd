...
AccaData %>% group_by(Date) %>% summarise(Wins = sum(Win), Return = 11*prod(Odds)) %>% ggplot(aes(x = Date)) + geom_bar(aes(y = Wins, fill = Return), stat = "identity") + geom_text(aes(label=ceiling(Return), y = Wins),vjust=-0.5, size = 2) + theme_classic() + labs(title = "Weekly tracker", subtitle = "Total estimated Return (£) from £11 stake shown above bar", caption = "bet365 boost odds on accas so the payout shown may be less than actual return on Doug's betslip") + ylim(c(0,11)) + scale_y_continuous(breaks = 0:11) + theme(panel.grid.major.y = element_line(linetype = 2)) #get summary of each acca

CloseCalls<- AccaData %>% group_by(Date) %>% summarise(Wins = sum(Win)) %>% filter(Wins == 10) %>% select(Date) %>% c()
library(kableExtra)
AccaData[which(AccaData$Date %in% CloseCalls$Date & AccaData$Win == 0),1:6] %>% kbl(caption = "Table of shame. When we were one result away from the jackpot", row.names = FALSE) %>% kable_classic(html_font = "Cambria") %>% kable_styling("striped")
