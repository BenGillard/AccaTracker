...
plotly::highlight(plotly::ggplotly(ggplot(data = d, aes(x = Date, y = Odds, col = Player)) + geom_point(aes(shape = as.factor(Win))) + geom_line() + theme_classic() + geom_hline(yintercept = 2, lty = 2, alpha = 0.5) + labs(title = "Tracker of bet odds over time", subtitle = "Click on a data point to see that player's trend (hold shift key for multiple selections", caption = "Dotted line represents \"evens\"") + theme(legend.position = "none")), on = "plotly_click", off = "plotly_relayout", color = "black", opacityDim = 0.3)
```
